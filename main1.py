...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        t_detect = time.time()
        dets = detect(img, model_detect, stride, device, imgsz)
        print(f"Time detect: {time.time() - t_detect}")
        t_depth = time.time()
        depth, disp_resized = predict(img1, model_depth)
        vmax = np.percentile(disp_resized, 95)
        plt.imsave(f"{name[:-4]}_depth.jpg", disp_resized, cmap='magma', vmax=vmax)
    ...
    for i in dets:
        ...
        distance = depth[(y1+y2)//2][(x1+x2)//2]
        print(distance)


        cv2.rectangle(img, (x1, y1), (x2, y2), (255, 255, 0), 2)
        cv2.putText(img, str(round(distance,1)) + "m", ((x1 + x2)//2, (y1 + y2)//2), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,0), 1)

    cv2.imwrite(f"{name[:-4]}_result.jpg", img)
    ...
    def click_event(event, x, y, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            print(f"depth: {depth_arr[y*2][x*2]}")
